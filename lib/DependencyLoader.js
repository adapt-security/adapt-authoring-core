...
class DependencyLoader {
  ...
  async load () {
    ...
    const { essential, theRest } = configValues.reduce((m, c) => {
      this.app.pkg.essentialApis.includes(c.essentialType) ? m.essential.push(c) : m.theRest.push(c)
      return m
    }, { essential: [], theRest: [] })
  ...
  async loadConfigs () {
    ...
    Object.entries(this.configs).forEach(([modName, config]) => {
      if (config.alias) {
        this.logWarning(`${config.name} will override ${config.alias}`)
        config.name = config.alias
        this.configs[config.alias] = config
        delete this.configs[modName]
      }
    })
  ...
  async loadModule (config) {
    const modName = config.alias || config.name
    const importPath = path.basename(config.rootDir)
    ...
    const { default: ModClass } = await import(importPath)
  ...
  async loadModules (configs) {
    await Promise.allSettled(configs.map(async c => {
      try {
        await this.loadModule(c)
      } catch (e) {
        this.logError(`Failed to load '${c.name}',`, e)
        const deps = this.peerDependencies[c.name]
        if (deps && deps.length) {
          this.logError('The following modules are peer dependencies, and may not work:')
          deps.forEach(d => this.logError(`- ${d}`))
        }
        this.failedModules.push(c.name)
      }
    }))
  }
  ...
  logWarning (...args) {
    if (this.app.logger && this.app.logger._isReady) {
      this.app.logger.log('warning', this.name, ...args)
    } else {
      console.log(...args)
    }
  }
