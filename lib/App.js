...
class App extends AbstractModule {
  ...
  async init () {
    ...
    try {
      ...
      await this.runMigrations(configFilePath)
   ...
   * @param {string} configFilePath Path to the user config file
   * @return {Promise}
   */
  async runMigrations (configFilePath) {
    let connectionUri
    try {
      connectionUri = (await import(configFilePath)).default?.['adapt-authoring-mongodb']?.connectionUri
    } catch {
      return
    }
    ...
    await runMigrations({
      ...
      connectionUri,
      rootDir: this.rootDir,
      log: (level, ...args) => this.logger.log(level, ...args),
