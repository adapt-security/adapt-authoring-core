...
class Config {
  ...
  static async load (rootDir, dependencies, appName) {
    ...
    if (userConfig) await instance.checkDeprecatedConfig(userConfig, dependencies)
  ...
  /**
   * Scans all dependencies for conf/deprecated*.json files and checks the user config for deprecated keys.
   * Each file maps old key names to new key names (or null if removed). Reserved keys:
   * - `_module`: check a different module section in the user config (for keys that moved between modules)
   * - `_fatal`: if true, throw an error instead of logging a warning when deprecated keys are found
   * @param {Object} userConfig The parsed user config object
   * @param {Object} dependencies Key/value map of dependency configs
   * @return {Promise}
   */
  async checkDeprecatedConfig (userConfig, dependencies) {
    const warnings = []
    let hasFatal = false
    await Promise.all(Object.values(dependencies).map(async dep => {
      const files = await glob('conf/deprecated*.json', { cwd: dep.rootDir, absolute: true })
      await Promise.all(files.map(async f => {
        try {
          const mappings = JSON.parse(await fs.readFile(f))
          const modName = mappings._module ?? dep.name
          const isFatal = mappings._fatal ?? false
          delete mappings._module
          delete mappings._fatal
          if (!userConfig[modName]) return
          for (const [oldKey, newKey] of Object.entries(mappings)) {
            if (!(oldKey in userConfig[modName])) continue
            if (isFatal) hasFatal = true
            if (newKey) {
              warnings.push(`  '${modName}.${oldKey}' has moved to '${newKey}'`)
            } else {
              warnings.push(`  '${modName}.${oldKey}' has been removed`)
            }
          }
