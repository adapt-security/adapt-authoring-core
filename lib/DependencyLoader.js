...
class DependencyLoader {
  ...
  /**
   * Loads all Adapt module dependencies in a single pass. Load ordering is handled by waitForModule. Fatal errors from any module will halt the application. loadConfigs() must be called before this method.
   * @return {Promise<void>}
   */
  async load () {
    const modules = Object.values(this.configs).map(c => c.name)
   ...
   * @returns {*|undefined} The configuration value, or undefined if config is not yet loaded
   */
  getConfig (key) {
