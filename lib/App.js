...
class App extends AbstractModule {
  ...
  async init () {
    ...
    try {
      ...
      this.log('verbose', 'DIR', 'dataDir', this.config.get('adapt-authoring-core.dataDir'))
      this.log('verbose', 'DIR', 'tempDir', this.config.get('adapt-authoring-core.tempDir'))
    } catch (e) {
      startError = e
    }
  ...
  async loadLibraries () {
     ...
     * @type {Config}
     */
    this.config = await Config.load(this.rootDir, deps, this.name)
    /**
     * Reference to the Logger instance
     * @type {Logger}
     */
    this.logger = new Logger({
      levels: this.config.get('adapt-authoring-core.loggerLevels'),
      mute: this.config.get('adapt-authoring-core.loggerMute'),
      dateFormat: this.config.get('adapt-authoring-core.loggerDateFormat'),
      showTimestamp: this.config.get('adapt-authoring-core.loggerShowTimestamp')
    })
    ...
    const configRootDir = this.config.get('adapt-authoring-core.rootDir')
