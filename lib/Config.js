...
class Config {
   ...
   */
  static async load (rootDir, dependencies, appName, logger) {
    ...
    logger.log('info', 'config', `using config at ${instance.configFilePath}`)
    return instance
  }
  ...
  async storeUserSettings () {
    ...
    if (configError) {
      this.logger.log('warn', 'config', `Failed to load config at ${this.configFilePath}: ${configError}. Will attempt to run with defaults.`)
      return
    }
  ...
  storeSchemaSettings (dependencies, appName) {
    ...
    if (errors.length) {
      errors.forEach(e => {
        this.logger.log('error', 'config', `${e.modName ? e.modName + ': ' : ''}${e.message}`)
      })
