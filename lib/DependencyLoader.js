...
class DependencyLoader extends Events {
   ...
   */
   static async loadModuleConfig(relPath) {
    const absPath = path.join(this.app.rootDir, relPath);
    return {
      ...await fs.readJson(path.join(absPath, Utils.packageFileName)),
      ...await fs.readJson(path.join(absPath, Utils.metadataFileName)),
      rootDir: absPath
    };
  }
  /**
