...
class Config {
   ...
   */
  static async load ({ rootDir, configFilePath, dependencies, appName, logger }) {
    ...
    logger.log('info', 'config', `using config at ${configFilePath}`)
    return instance
  }
  ...
  async storeUserSettings () {
    ...
    try {
      ...
      this.logger.log('warn', 'config', `Failed to load config at ${this.configFilePath}: ${e}. Will attempt to run with defaults.`)
      return
    }
