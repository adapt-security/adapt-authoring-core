...
class DependencyLoader extends Events {
  ...
  async load() {
    ...
    this.off('moduleloaded', this.handleModuleLoaded.bind(this));
    delete this.__waitListeners;
  }
  /**
  ...
  async loadConfigs() {
    ...
    const files = await globPromise(`${this.app.rootDir}/node_modules/**/${Utils.metadataFileName}`);
    const deps = files
    ...
    deps.forEach(d => {
      ...
      if(c.peerDependencies) {
        Object.keys(c.peerDependencies).forEach(p => this.peerDependencies[p] = [...(this.peerDependencies[p] || []), c.name]);
      }
    });
  ...
  async waitForModule(modName) {
    ...
    return new Promise((resolve, reject) => this.waitForModuleCb(modName, (e, instance) => e ? reject(e) : resolve(instance)));
  }
  /** @ignore */
  waitForModuleCb(name, cb) {
    if(!this.__waitListeners) this.__waitListeners = {};
    if(!this.__waitListeners[name]) this.__waitListeners[name] = [];
    this.__waitListeners[name].push(cb);
  }
  /** @ignore */
  handleModuleLoaded(modName, instance) {
    try {
      this.__waitListeners[modName].forEach(f => {
        const error = !instance && new Error(`dependency '${modName}' failed to load`) || undefined;
        f.call(this, error, instance);
      });
      delete this.__waitListeners[modName];
    } catch(e) {} // no problem, no listeners
  }
  /**
