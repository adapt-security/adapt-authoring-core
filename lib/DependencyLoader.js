...
class DependencyLoader extends Events {
  ...
  async loadModule(modName) {
    ...
    try {
      ...
      this.moduleLoadedHook.invoke(null, instance);
      return instance;
    } catch(e) {
      this.moduleLoadedHook.invoke(e);
      throw e;
    }
  }
  /**
  ...
  async waitForModule(modName) {
    ...
    return new Promise((resolve, reject) => {
      const wrapTap = (error, instance) => instance?.name === modName && (error ? reject(error) : resolve(instance));
      this.moduleLoadedHook.tap(wrapTap);
      this.app.onReady().then(() => this.moduleLoadedHook.untap(wrapTap));
    });
  }
  /**
