...
class Logger {
   ...
   * @param {Array<String>} options.levels Log level config strings
   * @param {Boolean} options.mute Whether to mute all output
   * @param {String} options.dateFormat Date format ('iso' or 'short')
   * @param {Boolean} options.showTimestamp Whether to show timestamps
   */
  constructor ({ levels = ['error', 'warn', 'success', 'info', 'debug', 'verbose'], mute = false, dateFormat = 'iso', showTimestamp = true } = {}) {
    /**
  ...
  static getDateStamp (config) {
    ...
    return Logger.colourise(`${str} `, chalk.dim)
  }
}
