...
class DataCache {
   ...
   * @param {Boolean} options.verboseLogs Whether to log cache hits and misses (default: false)
   */
  constructor ({ enable, lifespan, verboseLogs = false }) {
  ...
  async get (query, options, mongoOptions) {
    ...
    if (this.isEnabled && this.cache[key]) {
      this.hits++
      if (this.verboseLogs) App.instance.logger?.log('verbose', 'datacache', 'hit', options.collectionName, query)
      return this.cache[key].data
    }
    this.misses++
    if (this.verboseLogs) App.instance.logger?.log('verbose', 'datacache', 'miss', options.collectionName, query)
